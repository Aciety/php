...
RUN apt-get update -qq \
  ...
  && docker-php-ext-install -j$(nproc) pdo_mysql mysqli zip iconv intl bcmath curl exif opcache uuid \
  && pecl install APCu redis pcov \
  && docker-php-ext-enable apcu bcmath redis sodium pcov \
